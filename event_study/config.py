...
#   Constants
# --------------------------------------------------------
DATADTR = tk_cfg.DATADTR
FF_FACTORS_CSV = os.path.join(DATADTR, 'ff_daily.csv')
START = '1900-01-01'
END = '2020-12-31'
...
def csv_locs(tic):
    ...
    rec_csv = os.path.join(DATADTR, f'{tic}_rec.csv')
    prc_csv = os.path.join(DATADTR, f'{tic}_prc.csv')
    return {
            'rec_csv': rec_csv,
            'prc_csv': prc_csv,
            }
